<?php

namespace admin\shipping_charges\Console\Commands;

use Illuminate\Console\Command;
use Illuminate\Support\Facades\File;
use Illuminate\Support\Facades\Route;

class DebugShippingChargesCommand extends Command
{
    protected $signature = 'shipping_charges:debug';
    protected $description = 'Debug Shipping Charges module loading';

    public function handle()
    {
       $this->info('🔍 Debugging Shipping Charges Module...');
        
        // Check which route file is being loaded
        $this->info("\n📍 Route Loading Priority:");
        $moduleRoutes = base_path('Modules/ShippingCharges/routes/web.php');
        $packageRoutes = base_path('packages/admin/shipping_charges/src/routes/web.php');
        
        if (File::exists($moduleRoutes)) {
            $this->info("✅ Module routes found: {$moduleRoutes}");
            $this->info("   Last modified: " . date('Y-m-d H:i:s', filemtime($moduleRoutes)));
        } else {
            $this->error("❌ Module routes not found");
        }
        
        if (File::exists($packageRoutes)) {
            $this->info("✅ Package routes found: {$packageRoutes}");
            $this->info("   Last modified: " . date('Y-m-d H:i:s', filemtime($packageRoutes)));
        } else {
            $this->error("❌ Package routes not found");
        }
        
        // Check view loading priority
        $this->info("\n👀 View Loading Priority:");
        $viewPaths = [
            'Module views' => base_path('Modules/ShippingCharges/resources/views'),
            'Published views' => resource_path('views/admin/shipping_charge'),
            'Package views' => base_path('packages/admin/shipping_charges/src/resources/views'),
        ];
        
        foreach ($viewPaths as $name => $path) {
            if (File::exists($path)) {
                $this->info("✅ {$name}: {$path}");
            } else {
                $this->warn("⚠️  {$name}: NOT FOUND - {$path}");
            }
        }
        
        // Check controller resolution
        $this->info("\n🎯 Controller Resolution:");

        $controllers = [
            'ShippingMethodManagerController' => 'Modules\\ShippingCharges\\app\\Http\\Controllers\\Admin\\ShippingMethodManagerController',
            'ShippingRateManagerController' => 'Modules\\ShippingCharges\\app\\Http\\Controllers\\Admin\\ShippingRateManagerController',
        ];

        foreach ($controllers as $label => $controllerClass) {
            $this->info("Checking {$label}: {$controllerClass}");
            if (class_exists($controllerClass)) {
            $this->info("✅ Controller class found: {$controllerClass}");
            $reflection = new \ReflectionClass($controllerClass);
            $this->info("   File: " . $reflection->getFileName());
            $this->info("   Last modified: " . date('Y-m-d H:i:s', filemtime($reflection->getFileName())));
            } else {
            $this->error("❌ Controller class not found: {$controllerClass}");
            }
        }
        
        // Show current routes
        $this->info("\n🛣️  Current Routes:");
        $routes = Route::getRoutes();
        $shippingRoutes = [];

        foreach ($routes as $route) {
            $action = $route->getAction();
            if (isset($action['controller'])) {
            if (
                str_contains($action['controller'], 'ShippingMethodManagerController') ||
                str_contains($action['controller'], 'ShippingRateManagerController')
            ) {
                $shippingRoutes[] = [
                'uri' => $route->uri(),
                'methods' => implode('|', $route->methods()),
                'controller' => $action['controller'],
                'name' => $route->getName(),
                ];
            }
            }
        }
        
        if (!empty($shippingRoutes)) {
            $this->table(['URI', 'Methods', 'Controller', 'Name'], $shippingRoutes);
        } else {
            $this->warn("No shipping routes found.");
        }
    
    }
}
